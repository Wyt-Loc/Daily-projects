...
#define LCD_7725Buf_Size OV7725_WINDOW_WIDTH*OV7725_WINDOW_HEIGHT*2					//一帧传输的数据长度大小
static u8 lcd_7725buf[LCD_7725Buf_Size];	//一帧传输的数组缓存
static u8 first[4];
//更新LCD显示(OV7725)
	...
	if(ov_sta)//有帧中断更新
	{
		...
		for(i=0;i<OV7725_WINDOW_HEIGHT;i++)
		{
			for(j=0;j<OV7725_WINDOW_WIDTH;j++)
			{
				...
				OV7725_RCK_H; 
				LCD->LCD_RAM=color;
				lcd_7725buf[(i*(OV7725_WINDOW_WIDTH*2))+(j * 2)] = color >> 8;  //（2）（3）高八位  
				lcd_7725buf[(i*(OV7725_WINDOW_WIDTH*2))+((j * 2) + 1)] = color;	  //（2）（3）低八位				
			}
		}
//		first[0]=0x01;   
//	  first[1]=0xFE;	
//    Printf_Send(first,2);   //发送帧头
		
		USART_SendData(USART1,0x01);         //向串口3发送数据
		while(USART_GetFlagStatus(USART1,USART_FLAG_TC)!=SET){delay_us(100);};//等待发送结束
		USART_SendData(USART1,0xFE);         //向串口3发送数据
		while(USART_GetFlagStatus(USART1,USART_FLAG_TC)!=SET){delay_us(100);};//等待发送结束
		...
		USART_SendData(USART1,0xFE);         //发送帧尾
		while(USART_GetFlagStatus(USART1,USART_FLAG_TC)!=SET){delay_us(100);};//等待发送结束
		USART_SendData(USART1,0x01);         //向串口3发送数据
		while(USART_GetFlagStatus(USART1,USART_FLAG_TC)!=SET){delay_us(100);};//等待发送结束
...
{	 
	...
	uart_init(1000000);	 		//串口初始化为 115200
	//usart3_init(256000);
